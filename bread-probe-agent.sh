...
find_monitor_processes() {
    ...
    for pid in $pids; do
        if validate_pid "$pid"; then
            ...
            # 确认命令行确实包含我们的脚本
            if [[ "$cmd" =~ (bread-probe-service|bread-probe-agent) ]]; then
...
remove_autostart_settings() {
    ...
    local service_path
    if is_root_user; then
        service_path="/etc/systemd/system/bread-probe-agent.service"
    else
        service_path="$HOME/.config/systemd/user/bread-probe-agent.service"
    fi
    ...
    if [[ -f "$service_path" ]] && is_systemd_available; then
        ...
        $systemd_cmd stop bread-probe-agent.service 2>/dev/null || true
        $systemd_cmd disable bread-probe-agent.service 2>/dev/null || true
        rm -f "$service_path"
    ...
    if command_exists crontab; then
        ...
        if echo "$current_crontab" | grep -q "bread-probe-agent"; then
            echo "$current_crontab" | grep -v "bread-probe-agent" | crontab - 2>/dev/null
    ...
    for profile in "${profile_files[@]}"; do
        ...
        if [[ -f "$profile_path" ]] && grep -q "bread-probe-agent auto-start" "$profile_path" 2>/dev/null; then
            # FreeBSD兼容的sed语法
            if [[ "$OS" == "FreeBSD" ]] || [[ "$OS" == "Darwin" ]]; then
                sed -i '' '/# === bread-probe-agent auto-start BEGIN ===/,/# === bread-probe-agent auto-start END ===/d' "$profile_path" 2>/dev/null
            else
                sed -i '/# === bread-probe-agent auto-start BEGIN ===/,/# === bread-probe-agent auto-start END ===/d' "$profile_path" 2>/dev/null
            fi
...
EOF
        ...
        $systemd_cmd daemon-reload 2>/dev/null || true
        $systemd_cmd enable bread-probe-agent.service 2>/dev/null || true
    ...
    if command_exists crontab; then
        local current_crontab=$(crontab -l 2>/dev/null || echo "")
        if ! echo "$current_crontab" | grep -q "bread-probe-agent"; then
            local crontab_entry="@reboot sleep 30 && pgrep -f 'bread-probe-agent|bread-probe-service' >/dev/null || $SERVICE_FILE"
    ...
    if [[ -f "$profile" ]] && ! grep -q "bread-probe-agent auto-start" "$profile" 2>/dev/null; then
        cat >> "$profile" << EOF
# === bread-probe-agent auto-start BEGIN ===
# VPS监控服务自启动检测 (最后保障)
if [ -n "\$PS1" ] && [ "\$TERM" != "dumb" ]; then
    if ! pgrep -f 'bread-probe-agent|bread-probe-service' >/dev/null 2>&1; then
        (sleep 5 && nohup "$SERVICE_FILE" >/dev/null 2>&1 &) &
    fi
fi
# === bread-probe-agent auto-start END ===
EOF
...
[Unit]
Description=bread-probe-agent Service - VPS Monitoring Agent
Documentation=https://github.com/hhbread/bread-cloudflare-probe
...
EOF
    ...
    if is_root_user; then
        ...
        safe_systemctl enable bread-probe-agent.service
    else
        safe_systemctl --user daemon-reload
        safe_systemctl --user enable bread-probe-agent.service
    fi
...
start_service() {
    ...
    if [[ -f "$service_path" ]] && is_systemd_available; then
        ...
        if $systemd_cmd start bread-probe-agent.service 2>/dev/null; then
            $systemd_cmd enable bread-probe-agent.service 2>/dev/null || true
...
stop_service() {
    ...
    if [[ -f "$service_path" ]] && is_systemd_available; then
        ...
        if $systemd_cmd is-active bread-probe-agent.service >/dev/null 2>&1; then
            $systemd_cmd stop bread-probe-agent.service 2>/dev/null
            $systemd_cmd disable bread-probe-agent.service 2>/dev/null || true
            stopped=true
        fi
    fi
...
check_service_status() {
    ...
    if [[ -f "$service_path" ]] && is_systemd_available; then
        ...
        if $systemd_cmd is-active bread-probe-agent.service >/dev/null 2>&1; then
            ...
            $systemd_cmd status bread-probe-agent.service --no-pager -l 2>/dev/null || true
        else
    ...
    if is_root_user; then
        ...
        if [[ -f "$service_path" ]] && command_exists systemctl; then
            if systemctl is-enabled bread-probe-agent.service >/dev/null 2>&1; then
        ...
        if [[ -f "$service_path" ]] && command_exists systemctl; then
            if systemctl --user is-enabled bread-probe-agent.service >/dev/null 2>&1; then
                print_message "$GREEN" "    ✓ 服务已启用"
                active_count=$((active_count + 1))
            else
                print_message "$YELLOW" "    ✗ 服务未启用"
    ...
    if [[ -f "$HOME/.bashrc" ]] && grep -q "bread-probe-agent auto-start" "$HOME/.bashrc" 2>/dev/null; then
...
setup_profile_autostart() {
    ...
    # 检查是否已配置
    if grep -q "bread-probe-agent auto-start" "$profile" 2>/dev/null; then
        return 0
    fi
...
uninstall_monitor() {
    ...
    if [[ "$script_path" == "$SCRIPT_DIR"/* ]]; then
        need_backup=true
        local backup_script="/tmp/bread-probe-agent-backup.sh"
        ...
        while IFS=':' read -r type path action backup; do
            case "$type" in
                ...
                "crontab")
                    ...
                    (crontab -l 2>/dev/null || echo "") | grep -v "bread-probe-agent" | crontab - 2>/dev/null || true
                    ;;
                "profile")
                    print_message "$CYAN" "  清理profile修改: $path"
                    # 兼容FreeBSD的sed语法
                    if [[ "$OS" == "FreeBSD" ]] || [[ "$OS" == "Darwin" ]]; then
                        sed -i '' '/# === bread-probe-agent auto-start BEGIN ===/,/# === bread-probe-agent auto-start END ===/d' "$path" 2>/dev/null || true
                    else
                        sed -i '/# === bread-probe-agent auto-start BEGIN ===/,/# === bread-probe-agent auto-start END ===/d' "$path" 2>/dev/null || true
                    fi
                    ;;
